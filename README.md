...
**Week 3**
- Second activity, the detail activity that shows the user the details for a specific cat.
- Explicit intent from the main activity to the detail activity, carrying over the cat ID as an extra.
- Implicit intent on the share button on the main activity, to share the cat.
...
- The cat layout is defined in a separate file ```cat.xml```
- We use ```<include>``` to reuse the cat layout in our main activity
